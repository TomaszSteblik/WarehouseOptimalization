...
namespace OptimizationMethods.WarehouseMode
{
    public class Warehouse
    {
        public static void Optimizer(WarehouseParameters warehouseParameters)
        {
            ...
            GeneticAlgorithm geneticAlgorithm = new GeneticAlgorithm(warehouseParameters.WarehouseGeneticAlgorithmParameters, distancesMatrix,
                (population, matrix) =>
                {
                    double[] fitness = new double[population.Length];
                    Parallel.For((long) 0, population.Length, i =>
                    {
                        fitness[i] = Fitness.CalculateAllOrdersFitness(orders, population[i], distancesMatrix, warehouseParameters.WarehouseGeneticAlgorithmParameters);
                    });
